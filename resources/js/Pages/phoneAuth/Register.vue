<template>
    ...
    <form v-if="regSteps.getName" @submit.prevent="submitName">
        <div>
            <breeze-label for="name" value="Enter Name" />
...
export default {
    ...
    methods: {
        submitName(){
            this.form.post(this.route('registration'), {
                onSuccess: () => {
        ...
        submit() {
                ...
                if(this.form.phone.match(/^09\d{9}$/g)){
                    ...
                    this.form.post(this.route('phoneRegister'), {
                        onSuccess: () => {
                            return Promise.all([
